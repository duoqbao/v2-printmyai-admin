...
import DetailsData from "./Component/DetailsData";
...
export default function DetailsStyles() {
  const { id } = useParams();
  ...
  const getDataById = async (id) => {
    try {
      const res = await ApiStyle.getById(id);
      setData(res);
    } catch (error) {
      history.push("/styles");
    }
  ...
  useEffect(() => {
    document.title = "Details Avatar";
    getDataById(id);
    // setData(Data.datas[0]);
  }, [id, trigger]);
  const submitData = async () => {
    try {
      const res = await ApiStyle.update(dataEdit, id);
    ...
    return (
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <h6 className="font-semibold m-0">
          {isEdit ? "Edit Styles" : "Details Styles"}
        </h6>
        {isEdit ? (
        ...
        <div className="col-action">
          <Button
            onClick={() => {
              ...
              setAddStyle(tempData?.styles[index]);
                                ...
                                <div className="col-info">
                                  <Descriptions>
                                    ...
                                    <Descriptions.Item label="Seed" span={3}>
                                      {item?.attributes?.metadata?.seed ||
                                        "Null"}
                                    </Descriptions.Item>

                                    <Descriptions.Item label="Width" span={3}>
                                      {item?.attributes?.metadata?.width}
                                    </Descriptions.Item>
                                    <Descriptions.Item label="Height" span={3}>
                                      {item?.attributes?.metadata?.height}
                                    </Descriptions.Item>
                                ...
                                <div className="col-action">
                                  <Popconfirm
                                    ...
                                    onConfirm={async () => {
                                      ...
                                      try {
                                        const res = await ApiStyle.update(
                                          tempData,
                                          id
                                        );
                                        message.success("Successfully updated");
        ...
        <Modal
          ...
          onOk={async () => {
            ...
            if (dataEdit?.styles[indexExample]?.name) {
              ...
              try {
                const res = await ApiStyle.update(temp, id);
                message.success("Successfully updated");
                setIsAddStyle(false);
              } catch (error) {
                message.error("Failed to update");
